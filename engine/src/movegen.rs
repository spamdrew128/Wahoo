...
impl MoveGenerator {
    pub const fn new(killer: Move) -> Self {
        Self {
    ...
    fn add_move(&mut self, mv: Move) {
        if self.stage == MoveStage::QUIET && mv == self.killer {
            return;
        }
    ...
    fn generate_captures(&mut self, board: &Board) {
        ...
        into_moves!(|from|, normal_pawns, |to|, attacks::pawn(from, color).intersection(them), {
            self.add_move(Move::new(to, from, Flag::CAPTURE));
        });
    ...
    pub fn next<const INCLUDE_QUIETS: bool>(
        ...
        while self.stage_complete() {
            ...
            match self.stage {
                ...
                MoveStage::KILLER => {
                    if INCLUDE_QUIETS && self.killer.is_pseudolegal(board) {
                        self.add_move(self.killer);
                    }
                }
                MoveStage::QUIET => {
                    if INCLUDE_QUIETS {
    ...
    pub fn first_legal_move(board: &Board) -> Option<Move> {
        let mut generator = Self::new_empty();
